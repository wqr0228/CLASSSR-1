...
for test_loader in test_loaders:
    ...
    for data in test_loader:
        ...
        if need_GT:
            ...
            if gt_img.shape[2] == 3:  # RGB image
                ...
                logger.info(f'{img_name:20s} - PSNR: {psnr:.6f} dB; SSIM: {ssim:.6f}; PSNR_Y: {psnr_y:.6f} dB; SSIM_Y: {ssim_y:.6f}.')
                #logger.info(f'{img_name:20s} - PSNR: {psnr:.6f} dB;  PSNR_Y: {psnr_y:.6f} dB; .')
                # logger.info(
                #     '{:.6f}'.
                #         format(psnr_y))
    ...
    if need_GT:  # metrics
        ...
        #logger.info(f'----Average PSNR results for {test_set_name}----\n\tPSNR: {ave_psnr:.6f} dB; \n')
